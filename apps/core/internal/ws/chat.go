...
func HandleChatMessage(ctx *Context) error {
	...
	// We send the full message object so clients can render it immediately
	// Need to include sender info for the UI
	...
	// 4. Send to specific users via Hub
	for _, p := range conversation.Participants {
		// Hub handles checking if user is connected
