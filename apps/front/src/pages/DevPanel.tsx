...
export default function DevPanel() {
	...
	return (
		<div style={{ padding: '1rem', 'font-family': 'sans-serif' }}>
			...
			<Button variant="primary" onClick={() => navigate('/404')}>
				404
			</Button>
			...
			{status() && (
				<ul>
					...
					<li>Manifest reachable: {status().hasManifest ? '✅' : '❌'}</li>
					<li>
					...
					<li>
						Installable (seems to depends on random stuff like chrome / app
						already existing): {status().installPromptFired ? '✅' : '❌'}
					</li>
				</ul>
			)}
