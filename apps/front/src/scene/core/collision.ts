...
export function computeMovementFractionAABB(
	...
	let leftGap = ent.x + ent.width + dx - tar.x;
	let topGap = ent.y + ent.height + dy - tar.y;
	let rightGap = tar.x + tar.width - ent.x - dx;
	let bottomGap = tar.y + tar.height - ent.y - dy;

	if (dx > 0 && leftGap >= 0 && rightGap > 0 && topGap > 0 && bottomGap > 0)
		fx = Math.min(fx, (dx - leftGap) / dx);

	if (dx < 0 && leftGap > 0 && rightGap >= 0 && topGap > 0 && bottomGap > 0)
		fx = Math.min(fx, (rightGap - dx) / dx);

	if (dy < 0 && leftGap > 0 && rightGap > 0 && topGap > 0 && bottomGap >= 0)
		fy = Math.min(fy, (bottomGap - dy) / dy);

	if (dy > 0 && leftGap > 0 && rightGap > 0 && topGap >= 0 && bottomGap > 0)
		fy = Math.min(fy, (topGap - dy) / dy);
