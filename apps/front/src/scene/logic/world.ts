...
export class World {
    ...
    private recursiveCollide(
        ...
        if (isGroup(ent))
            for (const child of ent.getChildrens())
                if (child !== obs && child !== ent)
                    this.recursiveCollide(
                        child,
                        obs,
                        move,
                        absEX,
        ...
        if (isSolid(ent)) {
            ...
            move.dx =
                move.dx < 0 ? Math.max(move.dx, ax) : Math.min(move.dx, ax);
            move.dy =
                move.dy < 0 ? Math.max(move.dy, ay) : Math.min(move.dy, ay);
        }
    }
