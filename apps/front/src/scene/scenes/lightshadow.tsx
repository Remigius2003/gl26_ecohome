...
import { createEntity, createSolid, createCharacter, withInteractable, withDynamic } from "../logic/factory";
import { PhysicsSystem, PlayerController } from "../logic/movement";
import { drawDarknessWithLights } from "../logic/lighting";
import { GhostControllerNamed as GhostController, type DeviceLike } from "../logic/ghost";

// --- DEFAULT GAME PARAMS (tu ajusteras après) ---
const CELL = 96;
const UI_H = 42;

const SURGE_MAX_WATTS = 900;         // game over si >=
const WIN_TIME_MS = 60_000;          // win si survit 60s
const GHOST_COUNT_PER_LEVEL = [2, 3];
const GHOST_SPEED = 220;

type Device = (Entity & Solid) & DeviceLike & { lightRadius: number; label: string };
...
function generateWalls(map: string[], world: World) {
  let id = 0;
  for (let y = 0; y < map.length; y++) {
    for (let x = 0; x < map[y].length; x++) {
      if (map[y][x] !== "#") continue;
...
function createFloor(world: World, cols: number, rows: number) {
...
function makeDevice(
  ...
  colorOff: string,
  lightRadius: number,
  interactionCells: number = 1
): Device {
  // visible device (solid)
  ...
  base.toggle = (force?: boolean) => {
    ...
    base.isOn = next;
    base.text = new ColorTexture(next ? colorOn : colorOff, "rgba(255,255,255,0.6)");
  };

  // interaction zone (invisible, bigger)
  const pad = interactionCells * CELL;
  ...
  world.addEntity(withInteractable(zone, { onInteract: () => base.toggle(false) })); // player turns OFF
  world.addEntity(base);

  return base;
}

function makeExit(world: World, x: number, y: number, onNext: () => void) {
  const door = createEntity({
    ...
    height: CELL * 0.9,
    priority: 3,
    text: new ColorTexture("rgba(255,255,255,0.10)", "rgba(255,255,255,0.8)"),
  });
  ...
  const zone = createEntity({
    ...
    x: x - CELL * 0.5,
    y: y - CELL * 0.5,
    width: door.width + CELL,
    height: door.height + CELL,
    priority: -1,
    text: new ColorTexture("rgba(0,0,0,0)"),
  });
...
function drawBackArrow(ctx: CanvasRenderingContext2D, x: number, y: number, size: number) {
  ctx.save();
...
function drawBolt(ctx: CanvasRenderingContext2D, cx: number, cy: number, s: number) {
  ...
  ctx.beginPath();
  ctx.moveTo(cx - s * 0.10, cy - s * 0.48);
  ctx.lineTo(cx + s * 0.08, cy - s * 0.10);
  ctx.lineTo(cx - s * 0.02, cy - s * 0.10);
  ctx.lineTo(cx + s * 0.10, cy + s * 0.48);
...
function drawBulb(ctx: CanvasRenderingContext2D, cx: number, cy: number, s: number) {
  ctx.save();
  ...
  ctx.lineTo(cx - s * 0.16, cy + s * 0.20);
  ctx.moveTo(cx + s * 0.24, cy - s * 0.02);
  ctx.lineTo(cx + s * 0.16, cy + s * 0.20);
  ctx.stroke();
  ...
  ctx.moveTo(cx - s * 0.16, cy + s * 0.30);
...
export default class LightShadowScene implements Scene {
  ...
  init(canvas: HTMLCanvasElement, onSwitchScene: (t: SceneType) => void): void {
    ...
    const loadLevel = (idx: number) => {
      ...
      const map = MAPS[this.levelIndex % MAPS.length];
      const rows = map.length;
      const cols = map[0].length;

      this.world = new World(cols * CELL, rows * CELL);

      createFloor(this.world, cols, rows);
      generateWalls(map, this.world);

      const spawn = findSpawn(map);
      this.player = createCharacter({
        id: "player",
        x: spawn.x * CELL + CELL * 0.2,
        y: spawn.y * CELL + CELL * 0.2,
        width: CELL * 0.45,
        height: CELL * 0.45,
        speed: 600,
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          if (c === "E") {
            makeExit(this.world, x * CELL + CELL * 0.15, y * CELL + CELL * 0.05, () => {
              // next level
              const next = this.levelIndex + 1;
              // petite récompense si tu arrives avec peu de surtension
              if (this.surgeWatts < SURGE_MAX_WATTS * 0.5) this.score += 150;
              loadLevel(next);
            });
          }
        }
      ...
      // Ghosts
      const ghostCount = GHOST_COUNT_PER_LEVEL[this.levelIndex % GHOST_COUNT_PER_LEVEL.length] ?? 2;
      for (let i = 0; i < ghostCount; i++) {
        const g = withDynamic(
          createEntity({
            id: `ghost-${i}`,
            x: (cols - 2 - i) * CELL + CELL * 0.25,
            y: (rows - 2) * CELL + CELL * 0.25,
            width: CELL * 0.38,
            height: CELL * 0.38,
            priority: 3,
            text: new ColorTexture("rgba(255,255,255,0.35)", "rgba(255,255,255,0.7)"),
          }),
          { speed: GHOST_SPEED }
        );
        ...
        this.ghostControllers.push(
          new GhostController(
            ...
            this.world,
            () => this.devices,
            { speed: GHOST_SPEED, retargetEveryMs: 900, arriveDist: 26, interactMs: 650 }
          )
        );
      }
    ...
    // Optionnel : retour au menu principal via onSwitchScene("home")
    // (si tu veux brancher une UI "Back", tu peux l’appeler)
  }
  ...
  update(deltaTime: number): void {
    ...
    if (this.surgeWatts >= SURGE_MAX_WATTS) {
      this.gameOver = true;
      return;
    }

    // win condition: survive WIN_TIME_MS
    if (this.elapsed >= WIN_TIME_MS) {
      this.win = true;
      this.score += 500;
    }
  }
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    // lighting: GHOSTS + devices ON (PAS le joueur)
    const ghostLights = this.world.dynamics
      .filter(e => String(e.id).startsWith("ghost-"))
      .map(g => ({
        x: g.x + g.width / 2,
        y: g.y + g.height / 2,
        radius: 280, // taille lumière des ghosts (ajuste)
      }));
    ...
    const pillY = top;
    fillRoundRect(ctx, pillX, pillY, pillW, pillH, 18, "rgba(255,255,255,0.25)");
    strokeRoundRect(ctx, pillX, pillY, pillW, pillH, 18, "rgba(255,255,255,0.65)", 2);
    ...
    ctx.fillText(`LEVEL ${this.levelIndex + 1}   SCORE`, pillX + pillW / 2, pillY + pillH / 2);
    ctx.restore();
    ...
    fillRoundRect(ctx, bar1X, bar1Y, bar1W, bar1H, 8, "rgba(255,255,255,0.25)");

    const surgeRatio = Math.max(0, Math.min(1, this.surgeWatts / SURGE_MAX_WATTS));
    drawSegmentBar(
      ctx,
      bar1X + 4,
      bar1Y + 3,
    ...
    drawRingIcon(ctx, redRingCx, redRingCy, ringR, "rgba(255,70,70,0.95)", () => {
      drawBulb(ctx, redRingCx, redRingCy, ringR * 1.2);
    });

    fillRoundRect(ctx, bar2X, bar2Y, bar2W, bar2H, 8, "rgba(220,220,220,0.65)");
    ...
    ctx.textAlign = "left";
    ctx.textBaseline = "middle";
    ctx.fillText(`W ${Math.floor(this.surgeWatts)}/${SURGE_MAX_WATTS}`, bar1X + bar1W + 10, bar1Y + bar1H / 2);
    ctx.restore();
    ...
    if (this.gameOver || this.win) {
      ...
      ctx.textAlign = "center";
      ctx.font = "28px Arial";
      ctx.fillText(this.gameOver ? "GAME OVER" : "VICTOIRE", W / 2, H / 2 - 10);
